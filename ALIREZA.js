...
for (var i=0; i<30; i++) (function(t) {
 ...
 if ($('#topcmm-123flashchat-main-message-input').length) {
	...
	document.getElementById("topcmm-123flashchat-main-message-input").value=Math.random().toString(24).substr(2, 5);
    clearInterval(existCondition6);
    doTheRestOfTheStuff(parameters);
 }
}, 100); // check every 100ms
//existCondition7
var existCondition7 = setInterval(function() {
 if ($('#topcmm-123flashchat-main-send-msg-btn').length) {
    console.log("existCondition7 Exists!!");
	document.querySelector("#topcmm-123flashchat-main-send-msg-btn").click();
    clearInterval(existCondition7);
    doTheRestOfTheStuff(parameters);
 }
}, 100); // check every 100ms
...
var existCondition4 = setInterval(function() {
 if ($('#topcmm-123flashchat-loginview-username-input').length) {
	...
	document.getElementById("topcmm-123flashchat-loginview-username-input").value=Math.random().toString(36).substr(2, 6);
 }
}, 100); // check every 5000ms
//OK
//existCondition5
...
for (var i=0; i<30; i++) (function(t) {
 ...
 if ($('#topcmm-123flashchat-main-message-input').length) {
    console.log("existCondition6 Exists!!");
	document.getElementById("topcmm-123flashchat-main-message-input").value='*(&(&^&*&%^&$##@#$@#$@(*_';
	document.querySelector("#topcmm-123flashchat-main-send-msg-btn").click();
    clearInterval(existCondition6);
    doTheRestOfTheStuff(parameters);
 }
